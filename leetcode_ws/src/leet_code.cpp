  ...
  for (std::vector<int>::iterator iter = numbers.begin();
    ...
    while (iter1 < iter2 - 1) {
      std::vector<int>::iterator miditer =
          iter1 + static_cast<int>((iter2 - iter1) / 2);
...
bool LeetCode::checkInclusion(std::string s1, std::string s2) {
  ...
  //                 if(it2 ==s2_map.end()|| (it2!= s2_map.end() && it2->second
  //                 != it1->second))
  //
  //                     res = false;
  //                     break;
...
std::vector<int> LeetCode::sortedSquares(const std::vector<int> &nums) {
  int size = nums.size();
  std::vector<int> res;
  int pt1 = -1;
  int pt2 = size;
...
ListNode *LeetCode::transforArray2List(const std::vector<int> &nums) {
  if (nums.empty()) {
    return nullptr;
  } else {
...
ListNode *LeetCode::middleNode(ListNode *head) { return nullptr; }
